...
var _ hostservices.HostService = &KeyValueService{}
...
func (k *KeyValueService) HandleRequest(
	...
	if err != nil {
		k.log.Error("Failed to resolve host services KV bucket", slog.String("bucket", metadata[hostservices.BucketContextHeader]))
		code := uint(500)
...
func (k *KeyValueService) handleGet(
	...
	key := metadata[hostservices.KeyValueKeyHeader]
	if key == "" {
...
func (k *KeyValueService) resolveKeyValueStore(nc *nats.Conn, metadata map[string]string) (jetstream.KeyValue, error) {
	...
	kvStoreName := metadata[hostservices.BucketContextHeader]
