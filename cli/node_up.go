...
func (p nodeUpCmd) Run(ctx context.Context, logger *slog.Logger, cfg NexCLI) error {
	if cfg.Global.Check {
		return errors.Join(cfg.Global.Table(), cfg.Node.Table(), p.Table())
	}
	return nil
}

func (p nodeUpCmd) Validate() error {
