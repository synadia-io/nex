	...
	for workloadId, vm := range f.allVMs {
		...
		if vm.deployRequest != nil {
			pinfo := ProcessInfo{
				...
				OriginalRequest: vm.deployRequest,
			}
...
func (f *FirecrackerProcessManager) Stop() error {
	if atomic.AddUint32(&f.closing, 1) == 1 {
		...
		for vmID := range f.allVMs {
			...
			if err != nil {
				f.log.Warn("Failed to stop firecracker process", slog.String("workloadId", vmID), slog.String("error", err.Error()))
			}
		}
...
func (f *FirecrackerProcessManager) Start(procNotifier ProcessSubscriber) error {
	f.procNotifier = procNotifier
	...
	for !f.stopping() {
		...
		default:
			...
			go f.procNotifier.OnProcessReady(vm.vmmID)
...
func (f *FirecrackerProcessManager) StopProcess(workloadId string) error {
	...
	f.log.Debug("Attempting to stop virtual machine", slog.String("workloadId", workloadId))
