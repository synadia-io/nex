...
- **Update**: `nex --namespace default workload update <workload_id> --nexfile Nexfile` replaces the workload's definition in place — same workload ID, same node. Supply the replacement definition with `--nexfile`/`-f`, or place a `Nexfile` in the working directory; one of the two is required. The node validates the new `start_request` against the nexlet's schema, persists it, stops the running instance, and — only once that stop is confirmed — starts the replacement with freshly minted credentials.
  ...
  - If the stop can't be confirmed within the node's budget, the command reports `updated:false` with a message explaining why. The stop has already been dispatched and may still complete on the nexlet, leaving the workload stopped without a replacement; on a node with `--state` the new definition is already persisted and takes effect the next time the nexlet registers (an agent or node restart, not a timer), while on a stateless node (the default) the message tells you to redeploy.
  - If the stop is confirmed but the replacement fails to start, you also get `updated:false`: the old instance is stopped, and nothing is currently running. The same state rule applies — a stateful node finishes the update on the next agent registration, a stateless one requires a redeploy.
  - Because the new definition is persisted before the old instance is stopped, a node crash mid-update on a stateful node always completes on the next agent registration; it never reverts to the old definition.
- **Restart**: `nex --namespace default workload restart <workload_id>` stops and restarts the workload using its last **stored** definition, minting fresh credentials for the new instance. If a previous update left the stop unconfirmed or the replacement failed to start, the stored definition is already the new one, so restart finishes that interrupted update instead of reapplying whatever happened to still be running. When nothing is on file — a node running without `--state`, or a deploy whose record was never written — restart replays the definition the nexlet is currently running (and persists it, on a stateful node).
...
- A successful update or restart keeps the workload ID (and, for containers, the container *name*) stable, so the workload's own output can look unchanged. To confirm the bounce actually happened, check the process PID / container ID, the start time in `workload list`, or the `WORKLOADSTOPPED`/`WORKLOADSTARTED` event pair — see [Workload Control API](./control-api.md#verifying-that-a-verb-actually-acted).
